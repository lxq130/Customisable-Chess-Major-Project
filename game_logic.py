...
class Game:
    ...
    def in_board(self, coords):
        ...
        if (
            coords[0] >= 0
            and coords[0] < len(self.board)
            and coords[1] >= 0
            and coords[1] < len(self.board)
        ):
            return True
        return False
    ...
    def pawn_promote(self, move, promoted_piece):
        ...
        self.switch_turn()
        pieces = {
            "Knight": Knight(self.turn),
            "Bishop": Bishop(self.turn),
            "Queen": Queen(self.turn),
            "Rook": Rook(self.turn),
        }
        # Promotes the pawn into the promoted piece
        self.board[move.coords_f[1]][move.coords_f[0]] = pieces[promoted_piece]
    ...
    def in_check(self, colour):
        """Returns true if the given colour's king can be taken in one move."""
        for y, row in enumerate(self.board):
            for x, piece in enumerate(row):
    ...
    def validate_move(self, move):
        ...
        if self.board[move.coords_i[1]][move.coords_i[0]].name != "Empty":
            if (
                move in piece.get_all_moves(self, move.coords_i)
                and self.turn == piece.colour
            ):
