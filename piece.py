...
class Piece:
    ...
    def get_linear_moves(self, directions, game, coords, sight_range=-1):
        """Since the rook, bishop and queen have similar movement patterns, only one method is required for all three."""
        # Note that sight range refers to how many squares in each direction the piece can move in. -1 means it can move infinitely along its axes.
        inf_range = True if sight_range == -1 else False
        board = game.board
        possible_moves = []
        for direction in directions:
            i = 0
            while i < sight_range or inf_range:
                i += 1
                final_coords = (
                ...
                if game.in_board(final_coords):
                    ...
                    if final_piece.name != "Empty":
                        ...
                        break  # Stop searching if it finds a piece
                    else:
                        possible_moves.append(Move(coords, final_coords, board))
                else:  # Stop searching if out of board
                    break
...
class Pawn(Piece):
    def __init__(self, colour):
        ...
        self.transform_into = None  # Used in pawn promotions (Usually a name_colour)
    ...
    def get_all_moves(self, game, coords):
        ...
        if game.in_board(final_coords):
            ...
            if final_piece.name == "Empty":
                ...
                if game.in_board(final_coords):
                    final_piece = board[final_coords[1]][final_coords[0]]
                    if not self.has_moved(game.move_log) and final_piece.name == "Empty":
                        possible_moves.append(Move(coords, final_coords, board))
...
class King(Piece):
    ...
    def get_castle_moves(self, game, coords):
        """Returns a list of possible castle moves if any."""
        self.called = True
        board = game.board
        possible_moves = []
            ...
            for direction in range(-1, 2, 2):  # (direction = -1 then 1)
                ...
                while game.in_board((current_x, y)):
                    ...
                    if piece.name != "Empty" and piece.name != "King":
                        # Checks if the rook is at the last tile, that it hasn't moved and is not under attack.
                        if (
                            piece.name_colour == f"{self.colour[0]}Rook"
                            and not game.in_board((current_x + direction, y))  # Checks if the rook is on a corner
                        ...
                        # If it is not a rook that can be castled, break.
                            ...
                            break
                    # Otherwise check if the square is under attack. If it is castling is illegal.
...
class Witch(Piece):
    def __init__(self, colour):
        super().__init__("Witch", colour)

    def get_all_moves(self, game, coords):
        """Returns a list of move"""
        board = game.board
        directions_1 = [(1, 1), (-1, -1), (1, -1), (-1, 1)]
        directions_2 = [(0, 1), (1, 0), (0, -1), (-1, 0)]
        possible_moves = []
        for index, directions in enumerate([directions_1, directions_2]):
            # sight = index + 2
            for direction in directions:
...
ALL_PIECES = {
    ...
    "wRook": Rook("white"),
    "wWitch": Witch("white"),
}  # First character denotes colour, rest denote piece name
...
def get_piece(name_colour):
    ...
    if name_colour == "wPawn":
        return Pawn("white")
    if name_colour == "wWitch":
        return Witch("white")
    if name_colour == "bWitch":
        return Witch("black")
