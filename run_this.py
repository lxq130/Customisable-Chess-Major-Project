...
class Main:
    def __init__(self):
        ...
        self.bg = p.image.load(os.path.join("assets", "icons", "background.png"))
        self.screen = p.display.set_mode((WIDTH, HEIGHT))
        icon = p.image.load(os.path.join("assets", "icons", "chess_icon.png"))
    ...
    def menu(self):
        ...
        while True:
            ...
            chess_button_2 = Button(
                image=p.image.load(os.path.join("assets", "icons", "play_bg.png")),
                pos=(margin(WIDTH) + 24, menu_length + 200),
                text_input="CLASSIC",
                alignment="topleft",
                font=get_font(30),
                base_colour=RED_1,
            ...
            custom_button_2 = Button(
                image=p.image.load(os.path.join("assets", "icons", "play_bg.png")),
                pos=(1.5 * margin(WIDTH) + menu_length + 24, menu_length + 200),
                text_input="CUSTOM",
                alignment="topleft",
                font=get_font(30),
                base_colour=RED_1,
            ...
            for event in p.event.get():
                if event.type == p.QUIT:
                    ...
                    sys.exit()
                if event.type == p.MOUSEBUTTONDOWN:
                    if chess_button.check_input(mouse_pos) or chess_button_2.check_input(mouse_pos):
                        sounds.click.play(sounds.sfx_on)
                        self.chess(classic=True)
                    if customiser_button.check_input(mouse_pos) or custom_button_2.check_input(mouse_pos):
                        sounds.click.play(sounds.sfx_on)
                        self.customiser()

            p.display.update()

    def chess(self, classic):
        if classic:
            self.board_config.set_default()
        chess = Chess(self.sounds, self.board_config, classic)
    ...
    def game_over(self, winner):
        ...
        while True:
            ...
            if winner != "Draw":
                ...
                # Draw (usually because of stalemate)
                draw_text(
                    screen,
                    size=30,
                    text="Draw",
                    pos=(WIDTH // 2, (HEIGHT - HEIGHT_1) // 2 + HEIGHT_1 * 9 // 20),
                )
